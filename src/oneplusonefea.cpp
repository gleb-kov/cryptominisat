...
OnePlusOneFeaSAT::OnePlusOneFeaSAT(Solver* _solver) :
    solver(_solver)
{
}

OnePlusOneFeaSAT::~OnePlusOneFeaSAT()
{
    free(storebase);
    free(clause);
    free(clsize);
...
void OnePlusOneFeaSAT::large_mutation()
{
...
void OnePlusOneFeaSAT::small_mutation()
{
    bool updated = false;
    init_for_round();
...
void OnePlusOneFeaSAT::flipvar(uint32_t toflip)
{
...
void OnePlusOneFeaSAT::init_for_round()
{
    numfalse = 0;
...
uint32_t OnePlusOneFeaSAT::countVarsToFlip()
{
...
void OnePlusOneFeaSAT::pickflips(uint32_t flipsCnt)
{
...
template<class T>
OnePlusOneFeaSAT::add_cl_ret OnePlusOneFeaSAT::add_this_clause(const T& cl, uint32_t& i, uint32_t& storeused) {
    uint32_t sz = 0;
    bool sat = false;
...
bool OnePlusOneFeaSAT::init_problem()
{
